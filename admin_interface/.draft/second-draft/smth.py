def add_modsec_rule(rule_text, file_path):
    with open(file_path, 'a') as f:
        f.write(f"{rule_text}\n")

### I'll delete this since it will be interactive. Or just let it ###
import re

MODSEC_RULE_PATTERN = re.compile(r'^SecRule\s+.+')

def is_valid_modsec_rule(rule):
    return bool(MODSEC_RULE_PATTERN.match(rule.strip()))
#####################################################################
import subprocess
...
def update_rule(rule_id, new_rule):
    lines = list_rules()
    #search rule line by id
    lines[line_number] = new_rule + "\n"
